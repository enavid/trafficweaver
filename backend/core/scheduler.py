"""
    scheduler.py — Distribute N events across 24 hours following human-like
    activity patterns (low at night, peaks during morning/evening).
"""
...
def _weight_for_hour(hour: int, weights: List[float]) -> float:
    """Map an hour (0-23) to its 6-hour bucket weight."""
...
def generate_event_times(
    ...
    A small random jitter (+/-0 to 90 seconds) is applied to every
    timestamp so that no two events fall at identical seconds.
    """
    if base is None:
        base = datetime.now()
